...
class CustomDataset(torch.utils.data.Dataset):
	...
	def __init__(self,
		...
		self.labels = pd.read_csv(label_path) if label_path is not None and os.path.isfile(label_path) else None
	...
	def __getitem__(self, index):
		while True:
			try:
				vid = self.data[index]
				subpath = os.path.join(self.data_path, vid) 
				video_path = [os.path.join(subpath, vi) for vi in os.listdir(subpath) if vi.endswith(".mp4")]

				if len(video_path) > 0:
					path = random.choice(video_path)
					blackbak_crop = None
